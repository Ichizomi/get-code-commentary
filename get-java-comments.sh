...
# 2) - run this script via terminal and send the file name.java as first argument, send the desired 
...
truename=$(echo ${inputfile} | grep -o "[^\/]*\.java") # Grab true file name without path
...
pcregrep -M -o "([\"\'\`].*[\"\'\`])|(\/\*([^*]|(\*+[^*/]))*\*+\/)|(\/\/.*)" "${inputfile}_DOS" >${outputfile}
